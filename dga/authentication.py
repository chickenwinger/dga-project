    ...
    if request.method == "POST":
        ...
        try:
            ...
            if db.child("records").child(user["localId"]).get().val() is None:
                # User record doesn't exist, creating a template
                dga_typelist = ["dt1", "dt4", "dt5", "dp1", "dp2"]
                new_data = dict(zip(dga_typelist, [0] * len(dga_typelist)))

                db.child("records").child(user["localId"]).set({"2023": new_data})

            return redirect(url_for("graph.overview", type="dt1"))
        except:
